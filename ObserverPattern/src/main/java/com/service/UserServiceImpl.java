...
public class UserServiceImpl implements UserService{
	...
	@Override
	public UserMaster getUser(String name) throws UserNotFoundException {
		return Optional.ofNullable(userrepo.findByUsername(name))
				.map((n)->n)
				.orElseThrow(()->new UserNotFoundException("Invalid name..."));
	}
	
	public UserMaster getUserById(int uid) throws UserNotFoundException {
		Optional opt=userrepo.findById(uid);
		UserMaster user=(UserMaster)opt.get();
		return Optional.ofNullable(user)
				.map(n->n)
				.orElseThrow(()->new UserNotFoundException("invalid id.."));
	}
	
	public UserMaster checkUserByNameAndPassword(String username,String userpass)throws UserNotFoundException {
